...
import {
  ...
  CoinRouteParams,
  CoinRouteState,
} from "../interfaces/CoinDetail.interface";
import {
  BodyWrapper,
  Overview,
  OverviewItem,
  Tab,
  Tabs,
} from "../styledComponets/CoinDetail.styled";
import { Header, Title, Wrapper } from "../styledComponets/Coins.styled";
import {
  LoadingImage,
  LoadingWrapper,
} from "../styledComponets/Loading.styled";
...
function CoinDetail() {
  ...
  const {
    ...
    ["info", id],
    fetchCoinAPI.bind(`coins/${state?.coinId ?? id}`)
  );
  const {
    isLoading: isTickersLoading,
    error: tickersLoadError,
    data: tickersData,
  } = useQuery<CoinPrice>(
    ["tickers", id],
    fetchCoinAPI.bind(`tickers/${state?.coinId ?? id}?quotes=krw,usd`),
  ...
  return (
    <Wrapper>
      <Helmet>
        ...
        <meta
          name="description"
          content={`This page displays detailed information about ${capitalizeTitle(
            id
          )}.`}
        />
      </Helmet>
      <Header>
        <Title>
          {state?.coinName ?? (isLoading ? "Loading..." : infoData?.name)}
        </Title>
      </Header>
      <PrevButton />
      {isLoading ? (
