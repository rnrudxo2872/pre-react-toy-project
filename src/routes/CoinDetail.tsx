...
function CoinDetail() {
    ...
    const {isLoading:isTickersLoading, error:tickersLoadError, data:tickersData} = useQuery<CoinPrice>(["tickers",id], fetchCoinAPI.bind(`tickers/${state?.coinId ?? id}?quotes=krw,usd`));
    ...
    function renderCoinInfo() {
        return (
            <BodyWrapper>
                ...
                <Switch>
                    <Route path={`/${id}/price`}>
                        <Price krw={tickersData?.quotes.KRW} usd={tickersData?.quotes.USD}/>
                    </Route>
                    <Route path={`/${id}/chart`}>
                        <Chart coinId={id}/>
                    </Route>
